...
int main() {
    int rows, cols, colNum, startIndex;
    ...
    // Input start index
    printf("Enter start index for reversal (1 to %d): ", rows);
    scanf("%d", &startIndex);
    if (startIndex < 1 || startIndex > rows) {
    ...
    // Reverse the column from start index
    reverseArrayFromIndex(temp, rows, startIndex);
    ...
    printf("\nMatrix after reversing column %d from index %d:\n", colNum + 1, startIndex + 1);
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            printf("%d\t", matrix[i][j]);
        }
